...
const folderId = '<<レポートファイルを保存する親フォルダID>>'; // レポートファイルを保存する親フォルダIDを指定
const sheetId = '<<レポートを出力するスプレッドシートID>>'; // レポートを出力するスプレッドシートIDを指定
// 実行日の前日のメールを検索
// DMARC レポートを取得できる検索条件を指定する
const query = '<<検索条件>> after:' + Utilities.formatDate(targetDate, Session.getScriptTimeZone(), 'yyyy/MM/dd') + ' before:' + Utilities.formatDate(nextDate, Session.getScriptTimeZone(), 'yyyy/MM/dd');
...
function parseDMARCReports() {
  ...
  // 新しいシートを作成 ※左から2番目にシートを作成
  let sheet = spreadsheet.insertSheet(formattedDate, 1);
  ...
  let totalFiles = folder.getFiles().length;
  ...
  while (files.hasNext()) {
    ...
    processedFiles++;
    let progress = ((processedFiles / totalFiles) * 100).toFixed(2);
    Logger.log('Progress: ' + progress + '%');
  }
}
...
function calculateAndAppendHeaderFromTotals() {
  ...
  for (let headerFrom in headerFromTotals) {
    ...
    let quarantineRejectRateStr = quarantineRejectRate.toFixed(2) + '%'; // 小数点第2位まで表示
