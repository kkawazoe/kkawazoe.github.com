...
また、以下のスクリプトを AWS Console 上で実行することで inactive => active に状態を更新できる
...
AWS CLI で実行する場合は以下を参考にすること

```sh:make_active_all_lambda.sh
#!/bin/sh

# Shared
AWS_PROFILE_PREFIX={AWS プロファイルのプレフィックス}
LOG_FILE=log/make_active_all_lambda.log
...
function publish_lambda() {
    ...
    for lambda_name in ${inactive_lambda_name_list[@]}; do
        ...
        aws lambda --profile=${AWS_PROFILE_PREFIX}-${input_environment} publish-version --function-name $lambda_name
